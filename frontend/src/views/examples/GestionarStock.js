...
const GestionarStock = () => {  
  ...
  const handleSubmit = async (e) => {  
    ...
    let newQuantity = currentAsset.cantidad || 0;  
  
    if (movementType === "entrada") {  
      newQuantity += qty;  
    } else if (movementType === "salida") {  
      if (currentAsset.cantidad < qty) {  
        setMensaje({ tipo: 'danger', texto: 'No hay suficiente stock para la salida.' });  
        return;  
      }  
      newQuantity -= qty;  
    }  
    ...
    if (success) {  
      setMensaje({   
        tipo: 'success',   
        texto: `Stock de ${currentAsset.nombre} actualizado a ${newQuantity}`   
      });  
              ...
              <CardBody>  
                <Form onSubmit={handleSubmit}>  
                  ...
                  <div className="text-center">  
                    ...
                    >  
                      {loadingSubmit ? 'Procesando...' : 'Aplicar Movimiento'}  
                    </Button>  
                  </div>  
                </Form>  
              </CardBody>  
